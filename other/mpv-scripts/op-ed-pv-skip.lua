...
local function decrement_to(t)
	if t.to.value == 0 or t.from.value == t.to.value - 1 then
		return
	end
...
function OP_ED_THRESHOLD.from:increment()
	increment_from(OP_ED_THRESHOLD)
end
function OP_ED_THRESHOLD.from:decrement()
	decrement_from(OP_ED_THRESHOLD)
end
function OP_ED_THRESHOLD.to:increment()
	increment_to(OP_ED_THRESHOLD)
end
function OP_ED_THRESHOLD.to:decrement()
	decrement_to(OP_ED_THRESHOLD)
end
function PV_THRESHOLD.from:increment()
	increment_from(PV_THRESHOLD)
end
function PV_THRESHOLD.from:decrement()
	decrement_from(PV_THRESHOLD)
end
function PV_THRESHOLD.to:increment()
	increment_to(PV_THRESHOLD)
end
function PV_THRESHOLD.to:decrement()
	decrement_to(PV_THRESHOLD)
end
		...
		table.insert(menu, {
			...
			items = {
				string.format(
					...
					bool_to_integer(OP_ED_THRESHOLD.from.selected),
					OP_ED_THRESHOLD.from.value,
					bool_to_integer(OP_ED_THRESHOLD.to.selected),
					OP_ED_THRESHOLD.to.value
				),
				string.format(
					"- PV: {\\b%i}%i{\\b0}-{\\b%i}%i{\\b0} seconds",
					bool_to_integer(PV_THRESHOLD.from.selected),
					PV_THRESHOLD.from.value,
					bool_to_integer(PV_THRESHOLD.to.selected),
					PV_THRESHOLD.to.value
				),
			},
		})
	end
...
mp.observe_property("chapters", "number", function(_, chapter_count)
	if chapter_count == nil or chapter_count == 0 then
		return
	end
	...
	if
		(named_chapters and CURRENT_MODE ~= MODE_CHAPTER_NAME)
	...
	elseif CURRENT_MODE == MODE_CHAPTER_LENGTH then
		...
		if
			(
				...
				or chapter_index == last_chapter_index - 2
			) -- ../ED/PV/Endcard|
		...
		if
			chapter_index == last_chapter_index -- ../Preview|
