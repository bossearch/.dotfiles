...
while IFS= read -r file; do
  if [ -n "$file" ]; then
    ...
    echo "$extract" | xargs -r -I {} sh -c 'sleep 0.1; nohup {} > /tmp/launcher_output.log 2>&1 & disown'
  fi
done <<< "$selected"

echo "Selected: $selected" >> /tmp/launcher_debug.log
echo "Command: $extract" >> /tmp/launcher_debug.log
