...
local options = {
	...
	cache_dir = utils.join_path(os.getenv("HOME"), ".cache/mpv/ytsub"),
}
	...
	if is_auto then
		...
		input.select({
			...
			submit = function(lang_id)
				load_autosub(langs[lang_id], subs[langs[lang_id]], j["id"], true)
			end,
		})
	end
end

mp.add_key_binding(options.load_autosub_binding, function()
	ytsub(false)
end)
mp.add_key_binding(options.autoload_autosub_binding, function()
	ytsub(true)
end)

mp.register_event("file-loaded", function()
	mp.command("script-binding youtube_sub/__keybinding2")
end)
