...
local config = {
	subsync_path = "/sbin/ffsubsync", -- Replace the following line if the location of ffsubsync differs from the defaults
}
...
local function sync_sub_fn()
	...
	if file_exists(subtitle_path) == false then
		notify(
			table.concat({ "Subtitle synchronization failed:\nCouldn't find ", subtitle_path or "subtitle file." }),
			"error",
			3
		)
		return
